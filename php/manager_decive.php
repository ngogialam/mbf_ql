...
function searchRoom($number2)
{
  ...
  if ($con) {
    $seq_no = 0;
    $query = "SELECT device.*, sys_room.name_room FROM device  JOIN sys_room ON sys_room.id_room = device.id_room WHERE id_room = '$number2'";    
    $result = mysqli_query($con, $query);
    while ($row = mysqli_fetch_array($result)) {
      $seq_no++;
      showDeviceRow($seq_no, $row);
...
?>
    <tr>
        ...
        <!-- <td>
            ...
            if ($con) {
                $name_user_manager = "";
                $query1 = "SELECT * FROM manager_user";
                $result1 = mysqli_query($con, $query1);

                echo '<select name="name_user_manager" id="id_user" class=" form-control pdm chosen-select col col-md-12" >';
                while ($row1 = mysqli_fetch_assoc($result1)) {
                    $id_user = $row1['id_user'];
                    $name_user_manager = $row1['name_user_manager'];
                    if ($id_user == $id_user) {
