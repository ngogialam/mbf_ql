...
function searchTeamSys(number1) {
    console.log('5sdgsdfhbfsghbfsdbhdf');
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
        if (xhttp.readyState = 4 && xhttp.status == 200)
            document.getElementById('suppliers_div').innerHTML = xhttp.responseText;
    };
    xhttp.open("GET", "php/manage_supplier.php?action=search1&number1=" + number1, true);
    xhttp.send();
...
function refresh() {
    ...
    xhttp.open("GET", "php/manage_supplier.php?action=refresh", true);
    xhttp.send();
}